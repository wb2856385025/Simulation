 ...
 * Creating  Time：       10/8/2019 4:59:42 PM
...
namespace ProVision.InteractiveROI
{
    ...
    public class GraphicContext
    {
        ...
        /// 应用图形变量的上下文到HALCON窗口
        /// </summary>
        /// <param name="hwndHandle">HALCON窗口句柄</param>
        /// <param name="settings">哈希表存储的图形变量上下文</param>
        public void ApplyGraphicSettings(HalconDotNet.HTuple hwndHandle, System.Collections.Hashtable settings)
        {
            string key = "";
            string valS = "";
            int valI = -1;
            ...
            try
            {
                while (this._ienumerator.MoveNext())
                {
                    ...
                    switch (key)
                    {
                        case GC_COLOR:  //设置指定颜色(非多颜色)
                            valS = (string)settings[key];
                            HalconDotNet.HOperatorSet.SetColor(hwndHandle, new HalconDotNet.HTuple(valS));
                            if (LastGCSettings.Contains(GC_COLORED))
                                LastGCSettings.Remove(GC_COLORED);
                            break;
                        case GC_COLORED: //设置指定颜色数量(非指定颜色)
                            valI = (int)settings[key];
                            HalconDotNet.HOperatorSet.SetColored(hwndHandle, new HalconDotNet.HTuple(valI));
                            if (LastGCSettings.Contains(GC_COLOR))
                                LastGCSettings.Remove(GC_COLOR);
                            break;
                        case GC_DRAWMODE: //设置指定画模式(填充/边缘)
                            valS = (string)settings[key];
                            HalconDotNet.HOperatorSet.SetDraw(hwndHandle, new HalconDotNet.HTuple(valS));
                            break;
                        case GC_LINEWIDTH: //设置指定线宽
                            valI = (int)settings[key];
                            HalconDotNet.HOperatorSet.SetLineWidth(hwndHandle, new HalconDotNet.HTuple(valI));
                            break;
                        case GC_LUT: //设置指定查询表配置
                            valS = (string)settings[key];
                            HalconDotNet.HOperatorSet.SetLut(hwndHandle, new HalconDotNet.HTuple(valS));
                            break;
                        case GC_PAINT: //设置指定渲染配置
                            valS = (string)settings[key];
                            HalconDotNet.HOperatorSet.SetPaint(hwndHandle, new HalconDotNet.HTuple(valS));
                            break;
                        case GC_SHAPE: //设置区域显示指定形状
                            valS = (string)settings[key];
                            HalconDotNet.HOperatorSet.SetShape(hwndHandle, new HalconDotNet.HTuple(valS));
                            break;
                        case GC_LINESTYLE: //设置指定线型
                            valH = (HalconDotNet.HTuple)settings[key];
                            HalconDotNet.HOperatorSet.SetLineStyle(hwndHandle, valH);
                            break;
                        default:
                            break;
                    }
        ...
        public void SetColorAttribute(string val)
        {
            ...
            SetGraphicAttribute(GC_COLOR, val);
        }
        ...
        public void SetColoredAttribute(int val)
        {
            ...
            SetGraphicAttribute(GC_COLORED, val);
        }
        ...
        {
            SetGraphicAttribute(GC_DRAWMODE, val);
        }
        ...
        {
            SetGraphicAttribute(GC_LINEWIDTH, val);
        }
        ...
        {
            SetGraphicAttribute(GC_PAINT, val);
        }
        ...
        {
            SetGraphicAttribute(GC_SHAPE, val);
        }
        ...
        {
            SetGraphicAttribute(GC_LINESTYLE, val);
        }
        ...
        {
            SetGraphicAttribute(GC_LUT, val);
        }
